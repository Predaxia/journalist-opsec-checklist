# -journalist-opsec-checklist
Operational security checklist for journalists, NGO workers, and high-risk professionals. Maintained by Predaxia.

# OPSEC Checklist for Journalists and High-Risk Professionals
 
A practical, field-tested operational security checklist for journalists, NGO workers, lawyers, military families, and anyone operating in environments where digital privacy is not optional.
 
Maintained by [Predaxia](https://predaxia.com) — privacy for people who have something to lose.
 
---
 
## Who This Is For
 
- **Journalists** working with confidential sources or in hostile environments
- **NGO workers** operating in countries with active surveillance
- **Lawyers** handling sensitive client data across jurisdictions
- **Military families** managing OPSEC while a member is deployed
- **Travelers** crossing borders with sensitive devices or data
- **Anyone in a divorce or legal dispute** where device access is contested
 
---
 
## Before You Travel (72 hours)
 
- [ ] Prepare a clean travel device — not a factory reset, a [properly prepared device](https://predaxia.com/travel-phone-setup-clean-device/)
- [ ] Enable full-disk encryption (FileVault on Mac, BitLocker on Windows, LUKS on Linux)
- [ ] Set a strong alphanumeric password (12+ characters) — not biometric only
- [ ] Back up your primary device to encrypted storage, then leave it at home
- [ ] Remove sensitive apps (Signal history, password managers with source data)
- [ ] Review what a [border agent can actually extract](https://predaxia.com/device-seized-what-they-can-extract/) from a seized device
- [ ] If using 1Password: enable Travel Mode to [hide sensitive vaults](https://predaxia.com/1password-review-for-journalists/)
 
## Communication Security
 
- [ ] Default messaging: [Signal with disappearing messages](https://predaxia.com/signal-vs-protonmail-vs-wire/) enabled
- [ ] Never contact sources from a work device, work email, or work network
- [ ] Understand that [email metadata exposes more than content](https://predaxia.com/journalist-arrested-because-of-email/)
- [ ] Strip EXIF data from all photos before sending — [a single photo got a source arrested](https://predaxia.com/single-photo-got-source-arrested/)
- [ ] Use a VPN you trust — we recommend [Mullvad](https://mullvad.net) or [Proton VPN](https://predaxia.com/proton-vpn-review-2026/)
 
## Device Security
 
- [ ] Check for [signs of monitoring](https://predaxia.com/is-your-phone-being-monitored/) if in a domestic dispute
- [ ] Understand that [Pegasus requires no user interaction](https://predaxia.com/pegasus-doesnt-need-you-to-click/)
- [ ] Separate identities: work device, personal device, operational device — [never mix them](https://predaxia.com/operational-identity-separation-opsec/)
- [ ] Know what you are protecting, from whom — [build your threat model](https://predaxia.com/how-to-build-threat-model/)
 
## If Your Device Is Seized
 
- [ ] Do not consent to a search unless legally required — [know the law](https://predaxia.com/laptop-border-search-warrant/)
- [ ] Know what they [extract in 6 hours](https://predaxia.com/device-seized-what-they-can-extract/) with forensic tools
- [ ] If in an NGO context, follow [field seizure protocol](https://predaxia.com/ngo-workers-phone-searched-border/)
 
## VPN Selection
 
Not all VPNs are equal. Most [free VPNs sell your data](https://predaxia.com/free-vpns-sell-your-data/). A VPN [won't save you](https://predaxia.com/vpn-wont-save-you-if-this-already-happened/) if your device is already compromised.
 
We tested and recommend:
- **Mullvad** — [review](https://predaxia.com/mullvad-vpn-review-2026/) — no email, no name, cash payment
- **Proton VPN** — [review](https://predaxia.com/proton-vpn-review-2026/) — audited, Secure Core routing
- **Comparison**: [Proton vs Mullvad](https://predaxia.com/proton-vs-mullvad-2026/)
 
We [refuse to recommend](https://predaxia.com/tools-we-refuse-to-affiliate-with/) NordVPN, Surfshark, or ExpressVPN.
 
## Complete Glossary
 
59 OPSEC and privacy terms explained for civilians: [predaxia.com/glossary](https://predaxia.com/glossary/)
 
---
 
## Field Notes — Recent Incidents
 
- [The FBI recovered deleted Signal messages from a seized iPhone](https://predaxia.com/fbi-signal-messages-ios-notification-database/) (April 2026)
- [The FBI seized a Washington Post reporter's phone with 1,200 Signal sources](https://predaxia.com/fbi-seized-journalist-phone-signal-sources/) (April 2026)
- [Hong Kong made it a crime to refuse device unlock at the border](https://predaxia.com/hong-kong-device-unlock-border-law-2026/) (April 2026)
 
---
 
## About Predaxia
 
[Predaxia](https://predaxia.com) is an independent publication covering operational security for civilians. We test tools under real conditions, document field failures, and write for people whose privacy is not theoretical.
 
**There is no perfect setup. Anyone selling you perfect is selling fear. The goal is simple: make yourself a harder target than the person next to you.**
 
---
 
MIT License
